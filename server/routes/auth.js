...
router.post('/user/setAvatar', async (ctx, next) => {
  let name = ctx.request.body.name;
  let url = ctx.request.body.url;
  ctx.body = await UserCont.setAvatarUrl(name, url);
  await next();
})

router.get('/user/getAvatar/:name', async (ctx, next) => {
  let name = ctx.params.name;
  ctx.body = await UserCont.getAvatarUrl(name);
  await next();
})
